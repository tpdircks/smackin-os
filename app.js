...
(function () {
  ...
  const T = {
    en: { dash:"Overview", home:"Dashboard", receive:"Receive", putaway:"Put-Away", move:"Move / Pick", produce:"Produce",
      ...
      grpMaintenance:"Maintenance", grpTeam:"Team & Ops", maintenance:"Maintenance", mtHint:"Track maintenance requests, repairs, and projects - what's active, what's blocking a pending item, and a completed history. Move each one Requested -> In Progress -> Waiting -> Done.",
    ...
    es: { dash:"Resumen", home:"Panel", receive:"Recibir", putaway:"Almacenar", move:"Mover / Sacar", produce:"Producir",
      ...
      grpMaintenance:"Mantenimiento", grpTeam:"Equipo y Ops", maintenance:"Mantenimiento", mtHint:"Rastree solicitudes de mantenimiento, reparaciones y proyectos - que esta activo, que bloquea un pendiente y un historial de completados. Mueva cada uno Solicitado -> En Curso -> Esperando -> Hecho.",
    ...
    pt: { dash:"Visao geral", home:"Painel", receive:"Receber", putaway:"Armazenar", move:"Mover / Separar", produce:"Produzir",
      ...
      grpMaintenance:"Manutencao", grpTeam:"Equipe e Ops", maintenance:"Manutencao", mtHint:"Acompanhe solicitacoes de manutencao, reparos e projetos - o que esta ativo, o que esta bloqueando um pendente e um historico de concluidos. Mova cada um Solicitado -> Em Andamento -> Aguardando -> Concluido.",
  ...
  function viewHome() {
    ...
    const dStyle = '<style>' +
      ...
      '.ordtile.ot-prod{border-left:5px solid #2E6FB5;background:rgba(46,111,181,.07)}.ot-prod .ot-bar i{background:#2E6FB5}.ot-prod .ot-pill{background:#2E6FB5}' +
  ...
  function prodStageBar(o) {
    const cur = PROD_STAGES.indexOf(o._stage); const done = o._stage === "Done";
    const steps = PROD_STAGES.map((s, i) =>
      '<span class="postep' + (i <= cur ? ' on' : '') + (i === cur && !done ? ' now' : '') + '">' + s + '</span>').join('<span class="posep">&rsaquo;</span>');
    return '<div class="postages">' + steps + '</div>';
  }
  function prodCard(o) {
    const col = prioColor(o.priority);
    const bins = o.bins || o.bins4 || o.bins15;
    const sizeTag = o.size ? '<span class="posize">' + esc(o.size) + '</span>' : '';
    const nextBtn = o._stage === "Done"
      ? '<button class="ghost sm" onclick="UI.prodReset(\'' + esc(o.po) + '\')">Reopen</button>'
      : '<button class="ot-order" style="background:' + col + '" onclick="UI.prodAdvance(\'' + esc(o.po) + '\')">Advance &rarr;</button>';
    const instr = (o.mix_instr ? '<div class="poinstr"><b>Mixing:</b> ' + esc(o.mix_instr) + '</div>' : '') +
      (o.pmac_instr ? '<div class="poinstr"><b>P-Mac:</b> ' + esc(o.pmac_instr) + '</div>' : '');
    return '<div class="pocard" style="border-left:5px solid ' + col + '">' +
      '<div class="pohd"><span class="poflav">' + esc(o.flavor) + '</span> ' + sizeTag +
      '<span class="poprio" style="background:' + col + '">' + esc(o.priority || "—") + '</span></div>' +
      '<div class="pometa">PO ' + esc(o.po) + ' &middot; ' + esc(o.date) + (o.deadline ? ' &middot; due ' + esc(o.deadline) : '') + '</div>' +
      '<div class="pometa"><b>' + (bins ? esc(String(bins)) + ' bins' : '') + '</b>' + (o.seed ? ' &middot; ' + esc(o.seed) + ' seed' : '') + (o.to ? ' &middot; ' + esc(o.to) : '') + '</div>' +
      instr + prodStageBar(o) +
      '<div class="poact">' + nextBtn + '</div></div>';
  }
  function viewProdOrders() {
    const style = '<style>' +
      '.pogrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px}' +
      '.pocard{border:1px solid rgba(128,128,128,.18);border-radius:12px;padding:12px;background:rgba(128,128,128,.05);display:flex;flex-direction:column;gap:6px}' +
      '.pohd{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.poflav{font-weight:800;font-size:15px}' +
      '.posize{font-size:11px;font-weight:700;padding:1px 7px;border-radius:8px;background:rgba(128,128,128,.2)}' +
      '.poprio{margin-left:auto;font-size:10px;font-weight:800;color:#fff;padding:2px 7px;border-radius:8px;white-space:nowrap}' +
      '.pometa{font-size:12px;opacity:.8}.poinstr{font-size:12px;background:rgba(128,128,128,.08);border-radius:7px;padding:5px 7px}' +
      '.postages{display:flex;align-items:center;flex-wrap:wrap;gap:3px;margin:4px 0}' +
      '.postep{font-size:10px;font-weight:700;padding:2px 6px;border-radius:7px;background:rgba(128,128,128,.15);opacity:.55}' +
  ...
  function prodOrdersDeptCard(dept) {
    const list = prodOrdersList().slice(0, 6);
    if (!list.length) return "";
    const rows = list.map(o => {
      const bins = o.bins || o.bins4 || o.bins15;
      return '<tr><td><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:' + prioColor(o.priority) + '"></span></td>' +
        '<td><b>' + esc(o.flavor) + '</b><div class="muted sm">PO ' + esc(o.po) + (o.deadline ? ' &middot; due ' + esc(o.deadline) : '') + '</div></td>' +
        '<td class="right">' + (bins ? esc(String(bins)) + ' bins' : '') + '</td>' +
        '<td class="sm">' + esc(o._stage) + '</td></tr>';
    }).join("");
    return '<div class="card"><div class="suprow"><h2 class="sub2" style="margin:0;flex:1">&#128221; Allen\'s Orders (double-check)</h2>' +
      '<a class="order sm" onclick="UI_go(\'prodorders\')" style="cursor:pointer">Open &rarr;</a></div>' +
      '<p class="hint" style="margin:2px 0 8px">Authoritative work orders from Allen &mdash; verify the board below against these.</p>' +
      '<div class="tblwrap"><table><thead><tr><th></th><th>Flavor</th><th class="right">Bins</th><th>Stage</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
  }
  ...
  function viewProdLog() {
    ...
    const header = '<div class="card"><div class="suprow"><h2 style="flex:1;margin:0">' + L("prodlog") + '</h2>' +
      ...
      '<button class="ghost sm" title="Previous day" onclick="UI.plDateShift(-1)">&#9664;</button>' +
  ...
  function deptDemandBoard(dept, withTabs) {
    ...
    return (withTabs ? dqTabs(dept) : '') + (isFul ? '' : prodOrdersDeptCard(dept)) + (isFul ? '' : ecMakeNextCard()) + todayCard + logCard + scheduledCard + resultsCard + buildNeedCard(isFul ? "ful" : "prod", "buildnow");
  }
  ...
  // Sub-navigation for the Demand family — one nav entry, tabs move between the views.
  function demandTabsBar(active) {
    const T = [["demand", "Board"], ["demandboard", "Orders"], ["demandsched", "Schedule"], ["demandimport", "Import"], ["ecomdemand", "E-Com"], ["forecast", "Forecast"]];
    return '<div class="ordtabs" style="margin-bottom:12px;flex-wrap:wrap">' +
  ...
  function render() {
    ...
    const dbar = DEMAND_FAMILY.indexOf(active) >= 0 ? demandTabsBar(active) : "";
    $("view").innerHTML = alertStrip(active) + dbar + (map[active] || viewHome)();
...
{c:'Warehouse Forms',d:[['Warehouse Receiving Log (WHSE_FORM_1.0)','XLSX',F+'1QMKNAspsVDpU69uiBvo8lX6rfnesifRu/view'],['Warehouse Shipping Log (WHSE_FORM_2.0)','XLSX',F+'1LvrNt2YMtGS6PRL0T8sB01k9TMJGDT8F/view'],['Warehouse HOLD Log (WHSE_FORM_3.0)','XLSX',F+'143i2PH43HFInUIL5Yo29P8nGVfUKB8C4/view']]},
...
function render(){var v=document.getElementById('view');if(!v)return;var q=(window.__qualQ||'').toLowerCase();var h='';
h+='<div class="card"><div class="suprow"><div><h2>Quality &amp; Food Safety (SQF)</h2><p class="hint">Monica &amp; Brittney\'s SQF library — links open the live file in Google Drive (always current, never a stale copy).</p></div><div><a href="'+ROOT+'" target="_blank" style="display:inline-block;background:#e8622d;color:#fff;text-decoration:none;padding:8px 14px;border-radius:8px;font-size:13px;font-weight:600">Open Drive folder \u2197</a></div></div>';
h+='<input id="qual-search" placeholder="Search documents..." style="width:100%;box-sizing:border-box;margin:10px 0 4px;padding:8px 10px;border:1px solid #ddd;border-radius:8px;font-size:13px" value="'+(window.__qualQ||'')+'">';
h+='</div>';
CATS.forEach(function(cat){var rows=cat.d.filter(function(x){return !q||x[0].toLowerCase().indexOf(q)!==-1||cat.c.toLowerCase().indexOf(q)!==-1;});if(!rows.length)return;h+='<div class="card" style="margin-top:10px"><h3 style="margin:0 0 8px;font-size:14px">'+cat.c+' <span style="color:#aaa;font-weight:400">('+rows.length+')</span></h3>';rows.forEach(function(x){h+='<div style="display:flex;align-items:center;padding:6px 0;border-top:1px solid #f0f0f0">'+badge(x[1])+'<a href="'+x[2]+'" target="_blank" style="color:#1a5fb4;text-decoration:none;font-size:13px;flex:1">'+x[0]+'</a><span style="color:#ccc;font-size:12px">\u2197</span></div>';});h+='</div>';});
v.innerHTML=h;var s=document.getElementById('qual-search');if(s){s.oninput=function(){window.__qualQ=s.value;var p=s.selectionStart;render();var s2=document.getElementById('qual-search');if(s2){s2.focus();try{s2.setSelectionRange(p,p);}catch(e){}}};}}
window.__qualRender=render;
function ensureNav(){var nav=document.getElementById('nav');if(!nav)return;if(document.getElementById('qual-nav-item'))return;var grp=document.createElement('div');grp.className='navgroup';grp.innerHTML='<button class="navlabel" style="pointer-events:none">QUALITY</button>';var b=document.createElement('button');b.id='qual-nav-item';b.className='navitem';b.innerHTML='<span>\uD83D\uDEE1 Quality (SQF)</span>';b.onclick=function(){window.__qualActive=true;[].slice.call(nav.querySelectorAll('.navitem')).forEach(function(n){n.classList.remove('active');});b.classList.add('active');render();};grp.appendChild(b);nav.appendChild(grp);}
var navClickCap=function(e){var t=e.target.closest?e.target.closest('.navitem'):null;if(t&&t.id!=='qual-nav-item'){window.__qualActive=false;}};
if(!window.__qualNavHook){document.getElementById('nav')&&document.getElementById('nav').addEventListener('click',navClickCap,true);window.__qualNavHook=true;}
ensureNav();
if(!window.__qualInt){window.__qualInt=setInterval(function(){ensureNav();if(window.__qualActive&&!document.querySelector('#view #qual-search')){render();}},800);}
})();
