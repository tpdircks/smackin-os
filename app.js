...
(function () {
  ...
  function poLinesFromText(lines){ var out=[],inItems=false; for(var i=0;i<lines.length;i++){ var ln=lines[i]; if(/item\s*#/i.test(ln) && /quantity/i.test(ln)){ inItems=true; continue; } if(!inItems) continue; if(/subtotal/i.test(ln)) break; var t=ln.trim(); if(!t || /^[\$\-0\s]+$/.test(t)) continue; var r=poParseItemLine(t); if(r && (r.item||r.desc) && r.qty) out.push(r); } return out; }
