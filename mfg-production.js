...
/* Smackin OS — Manufacturing: daily production numbers for Mixing & P-Mac (1.5oz + 4oz).
   First pass from Allen's "1736 - Production Tracking" (Detail tab: Date, Dept, Shift,
   Bag Size, Flavor, Box Count, Total Pkgs). Captures the four daily totals:
   Mixing 1.5oz / Mixing 4oz / P-Mac 1.5oz / P-Mac 4oz (in packages = bags).
   ...
   Stores to Supabase mfg_daily (see mfg_daily_table.sql). Refine after Allen replies
   (per-run vs daily total, Total Pkgs vs Box Count, Mixing unit bins vs bags). */
(function(){
  ...
  /* the four cells: key, dept, size */
  var CELLS=[
    {k:'mixing_15', dept:'Mixing', size:'1.5 oz'},
    {k:'mixing_4',  dept:'Mixing', size:'4 oz'},
    {k:'pmac_15',   dept:'P-Mac',  size:'1.5 oz'},
    {k:'pmac_4',    dept:'P-Mac',  size:'4 oz'}
  ];
  ...
  function formHTML(){
    ...
    return '<div class="mp-wrap">'+
      ...
      '<p>Enter today’s output for each line and bag size. Numbers are packages (bags) produced, from Allen’s tracking.</p></div>'+
      '<div class="mp-row2">'+
  ...
  function save(){
    ...
    var rec={ prod_date:date, shift:(document.getElementById('mp_shift')||{}).value||null,
      unit:'packages', notes:(document.getElementById('mp_notes')||{}).value||'',
    ...
    api('mfg_daily?on_conflict=prod_date',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify([rec])})
      ...
      .then(function(){ var t=rec.mixing_15+rec.mixing_4+rec.pmac_15+rec.pmac_4;
        msg.innerHTML='<div class="mp-ok">✓ Saved '+fdate(date)+' — '+fmt(t)+' packages total (Mixing '+fmt(rec.mixing_15+rec.mixing_4)+', P-Mac '+fmt(rec.pmac_15+rec.pmac_4)+').</div>'; })
    ...
    api('mfg_daily?select=*&order=prod_date.desc&limit=60').then(function(r){return r.json();}).then(function(rows){
      ...
      var cards=CELLS.map(function(c){
        return '<div class="mp-card mp-'+(c.dept==='Mixing'?'mix':'pmac')+'"><div class="mp-card-h">'+c.dept+' · '+c.size+'</div>'+
          '<div class="mp-card-n">'+fmt(n(t[c.k]))+'</div><div class="mp-card-s">'+fmt(wk[c.k])+' this week</div></div>';
      }).join('');
      var latest = t.prod_date? ('Latest: '+fdate(t.prod_date)) : 'No entries yet';
      var rowsHTML = rows.length? rows.map(function(r){
        var tot=n(r.mixing_15)+n(r.mixing_4)+n(r.pmac_15)+n(r.pmac_4);
        return '<tr><td>'+esc(fdate(r.prod_date))+'</td><td class="r">'+fmt(n(r.mixing_15))+'</td><td class="r">'+fmt(n(r.mixing_4))+'</td>'+
          '<td class="r">'+fmt(n(r.pmac_15))+'</td><td class="r">'+fmt(n(r.pmac_4))+'</td><td class="r b">'+fmt(tot)+'</td>'+
          '<td><button class="mp-btn ghost sm" data-d="'+esc(r.prod_date)+'">Edit</button></td></tr>';
      }).join('') : '<tr><td colspan="7" style="text-align:center;color:#6E7C8A;padding:20px">No production entered yet. Use Production Entry to add today’s numbers.</td></tr>';
      document.getElementById('mp_body').innerHTML=
        '<div class="mp-cards">'+cards+'</div>'+
        '<div class="mp-latest">'+latest+' · cards show the latest day; sub-line is the last 7 days.</div>'+
        '<table class="mp-tbl"><thead><tr><th>Date</th><th class="r">Mix 1.5</th><th class="r">Mix 4</th><th class="r">P-Mac 1.5</th><th class="r">P-Mac 4</th><th class="r">Total</th><th></th></tr></thead><tbody>'+rowsHTML+'</tbody></table>';
